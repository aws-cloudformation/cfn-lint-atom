...
describe('The CFN Lint provider for Linter', () => {
  ...
  describe('disable rule', () => {
    let editor = null;
    beforeEach( async () => {
        atom.config.set('atom-cfn-linter:ignoreRules', ['E3012'])
    });

    it('it doesn\'t return results for disabled rule', async () => {
      const editor = await atom.workspace.open(paths.badYaml)
      const messages = await lint(editor)
  ...
  describe('override spec', () => {
    let editor = null;
    beforeEach( async () => {
        atom.config.set('atom-cfn-linter:overrideSpecPath', paths.overrideSpec)
    });
  ...
  describe('append rules', () => {
    let editor = null;
    beforeEach( async () => {
        atom.config.set('atom-cfn-linter:appendRules', paths.appendRules)
    });
