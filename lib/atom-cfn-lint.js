      ...
      lint: (activeEditor) => {
        ...
        return helpers.exec(this.cfnLintExecutablePath, args, { cwd: require('path').dirname(file), ignoreExitCode: true }).then((output) => {
          JSON.parse(output).forEach((match) => {
            const linenumber = parseInt(match.Location.Start.LineNumber, 1000) - 1
            const columnnumber = parseInt(match.Location.Start.ColumnNumber, 1000) - 1
            const linenumberend = parseInt(match.Location.End.LineNumber, 1000) - 1
            const columnnumberend = parseInt(match.Location.End.ColumnNumber, 1000) - 1
